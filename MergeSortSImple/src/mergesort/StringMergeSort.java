...
public class StringMergeSort {
	...
	public static String stringMergeSort(String input) {
		...
		String newFirstHalf = stringMergeSort(input.substring(0, length / 2));
		String newSecondHalf = stringMergeSort(input.substring(length / 2, length));

		String sortedString = "";
		while (newFirstHalf.length() > 0 && newSecondHalf.length() > 0) {
			if (newFirstHalf.charAt(0) < newSecondHalf.charAt(0)) {
				sortedString = sortedString + newFirstHalf.charAt(0);
				newFirstHalf = newFirstHalf.substring(1);
			} else {
				sortedString = sortedString + newSecondHalf.charAt(0);
				newSecondHalf = newSecondHalf.substring(1);
			}
		}
		return sortedString + newFirstHalf + newSecondHalf;
	}
