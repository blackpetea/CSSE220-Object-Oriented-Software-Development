 ...
 * 
 * Note that you should feel free to add new methods (e.g. utility methods like
 * size() or recursive helper methods) if they will help you solve the problems.
 * 
...
public class StringLinkedList {
	 ...
	 * 
	 * "1"->"2"->"3"->"4" becomes "1"->"2" "a"->"b"->"c" becomes "a"->"b" "a"
	 * becomes "a"
	 * 
	 ...
	 * For example:
	 * 
	 * The list "apple"->"bacon"->"ninja" would return true This list
	 * "bigger"->"badder" would return false
	 * 
	...
	public boolean isSorted() {
		...
		while (current.next != null) {
			int length = (current.data.length() < current.next.data.length()) ? current.data.length()
					: current.next.data.length();
			for (int i = 0; i < length; i++) {
				if (current.data.charAt(i) < current.next.data.charAt(i)) {
					current = current.next;
				} else if (current.data.charAt(i) == current.next.data.charAt(i)) {
					i++;
				} else {
					return false;
				}
	...
	/**
	 * 
	 * Takes a list of strings and converts it to a list of strings such that
	 * each string is one (or zero) characters long.
	 * 
	 * So for example: "foo"->"bar" yields "f"->"o"->"o"->"b"->"a"->"r"
	 * "a"->""->"bc" yields "a"->""->"b"->"c"
	 * 
