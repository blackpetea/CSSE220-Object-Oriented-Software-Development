...
public class RecursionPractice {
	...
	public static String weirdoString(String stringInput) {
		...
		String last=stringInput.substring(1);
		if (first=='a'||first=='b') {
			return weirdoString(last);
		} else if (first=='C') {
			return "QQQ"+weirdoString(last);
		}
		return first+weirdoString(last);
	}
	...
	public static int arrayListSummation(ArrayList<Integer> inputList) {
		...
		inputList.remove(0);
		return num+arrayListSummation(inputList);
	}
	 ...
	 * a bunch of ending brackets (like this "([<>])"). You don't have worry
	...
	public static boolean evenPowerOfThree(int inputValue) {
		if (inputValue!=1&&inputValue%9!=0) {
			return false;
	 ...
	 * 
	 * @param inputString
	 * @return true if the string has not doubled letters, false if it has
	 *         doubled letters
	 */
	...
	public static boolean numbersGame(int startingNumber) {
		...
		if (startingNumber<8&&startingNumber!=3) {
			return false;
		}
		if (startingNumber%3==0) {
			boolean res=numbersGame(startingNumber/3);
			if (res==false) {
