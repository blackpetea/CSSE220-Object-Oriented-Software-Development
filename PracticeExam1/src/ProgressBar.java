...
public class ProgressBar {
	...
	private Rectangle2D.Double progb;
	private Rectangle2D.Double progc;
	private double per;
	private double sx;
	private double sy;
	...
	public ProgressBar() {
		this.sx=0;
		this.sy=0;
		this.bwidth=DEFAULT_HEIGHT;
		this.blength=DEFAULT_WIDTH;
		this.progb= new Rectangle2D.Double(this.sx,this.sy,this.blength,this.bwidth);
	}
	
	public ProgressBar(int startx,int starty,int width,int length) {
		this.sx=startx;
		this.sy=starty;
		this.blength=width;
		this.bwidth=length;
	...
	public void setPercentage(double percentage) {
		this.per=percentage/100;
		sxc=this.sx+BORDER_WIDTH;
		syc=this.sy+BORDER_WIDTH;
		cwidth=(double)this.blength-2*BORDER_WIDTH;
		clength=(double)this.bwidth-2*BORDER_WIDTH;
		this.progc=new Rectangle2D.Double(this.sxc, this.syc,this.cwidth*this.per,this.clength);
	}
	...
	public void drawOn(Graphics2D g2) {		
		...
		Double num1=this.per*100;
		String text=num1.toString()+"%";
		g2.translate(this.sx+BORDER_WIDTH, this.sy);
		g2.rotate(Math.toRadians(90));
		RectangleTextUtils.fillTextRectangle(0,0,(int)this.bwidth,BORDER_WIDTH,text,g2);
		g2.rotate(-Math.toRadians(90));
		g2.translate(-this.sx-BORDER_WIDTH, -this.sy);
