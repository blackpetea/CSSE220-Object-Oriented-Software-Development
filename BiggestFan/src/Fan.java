...
public class Fan {
	...
	public void drawOn(Graphics2D graphics2) {
		...
		double width=this.BLADE_ASPECT_RATIO*this.bladeLength;
		for (int i=0;i<this.numberOfBlades;i++) {
			double deg=360/this.numberOfBlades;
			Rectangle2D rect=new Rectangle2D.Double(-0.5*width, -this.bladeLength, width, this.bladeLength);
			graphics2.translate(this.centerX, this.centerY);
			graphics2.rotate(Math.toRadians(deg*i));
			graphics2.draw(rect);
			graphics2.rotate(-Math.toRadians(deg*i));
			graphics2.translate(-this.centerX, -this.centerY);
		}	
