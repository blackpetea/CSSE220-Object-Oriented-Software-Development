...
public class TempHumidityMonitorMain {
	...
	public static void main(String[] args) {
		...
		ArrayList<TemperatureHumiditySensor> monitors = createMonitors();
	 ...
	 * @return
	 */
	public static ArrayList<TemperatureHumiditySensor> createMonitors() {
		Random rand = new Random();
		ArrayList<TemperatureHumiditySensor> monitors = new ArrayList<TemperatureHumiditySensor>();
		ErrorReporter errorHandler = new ErrorReporter(new ArrayList<MonitorError>());
		...
		for (int i = 0; i < LabNames.length; i++) {
			...
			if (USE_TEST_SENSOR) {
				monitor = new TemperatureHumiditySensorWithButtons(errorHandler,labName, rand.nextDouble() * 50,
						rand.nextDouble() * 50 + 60, rand.nextDouble() * 30, rand.nextDouble() * 50 + 50);
			} else {
				monitor = new RandomTemperatureHumiditySensor(labName, rand.nextDouble() * 50,
						rand.nextDouble() * 50 + 60, rand.nextDouble() * 30, rand.nextDouble() * 50 + 50);
			}
