...
public class LinkedList<ListDataType> implements Iterable<ListDataType> {
	...
	private class Node<NodeDataType> {
		...
		public Node(ListDataType value2, Node<NodeDataType> next) {
			this.value = (NodeDataType) value2;
			this.next = next;
		}
	}
	...
	public void addAtBeginning(ListDataType value) {
		this.head = new Node<ListDataType>(value, this.head);
	}
	...
	public void removalAll(int nodesWithValue) {
		while (!this.head.equals(null) && this.head.value.equals(nodesWithValue)) {
			removeBeginning();
		}
		if (this.head == null)
			return;
	...
	public Iterator<ListDataType> iterator() {
		return new IIterator<ListDataType>(this.head);
	}

	private class IIterator<IteratorDataType> implements Iterator<IteratorDataType> {

		private Node<IteratorDataType> currentNode;

		public IIterator(Node<IteratorDataType> node) {
		...
		public IteratorDataType next() {
			IteratorDataType result=currentNode.value;
